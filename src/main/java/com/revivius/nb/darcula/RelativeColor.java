 ...
 * <ul>
 * <li>A base color, as defined in a UI spec - this might be the expected value for window titlebars, for example </li>
 * <li>A target color, as defined in a UI spec, whose color is not the base color, but has a relation to it (such as
 * brighter or darker, or hue shifted)</li>
 * <li>The actual color - which may differ from the base color if the user has customized their UI them (for example,
 * changing the color defaults in Windows)</li>
 * <li>(optional) A color that the result must contrast with sufficiently that text will be readable</li>
 * </ul>
 * When constructing the real value, a color will be generated which has the same relationship to the original value as
 * the base color has to the target color.
 *
 * <h2>What this class is for</h2>
 * A number of components in NetBeans have colors that should be based on a color taken from the desktop theme. Swing
 * provides a mechanism for getting these colors, via UIManager, which will supply correct colors based on the desktop
 * theme for a variety of operating systems.
 * <p>
 * But often the color in a UI specification is not the same as, but related to the color that should be used. For
 * example, in windows classic, the tabs have a gradient based on a light blue color. The color should be related to the
 * dark blue color normally used in Windows for window titles. However, if the user has set the window titlebar color to
 * red, a reddish color should be used.
 * <p>
 * This class allows you to provide a base value (the default Windows titlebar color, hardcoded) and a prototype value
 * (the blue color that should be used <i>if</i> the desktop colors are the defaults), and the <i>actual</i>
 * value retrieved from the UI. The instance of this class is then dropped into <code>UIDefaults</code>; code can simply
 * call <code>UIManager.getColor("someColor")</code> and get the right color without being cluttered with the details of
 * deriving colors.
 *
 * <h2>How it does what it does</h2>
 * The base and prototype are split into HSB color components. The relationship between the base and prototype values in
 * saturation and brightness is then computed. This same relationship is then applied to the actual value
 * <i>as a function of the divergence between the base and actual values</i>
 * such that the more a color diverges, the less the relationship is applied - so that, if the base color is dark blue
 * and the prototype color is light blue, but the actual color is light yellow, you get light yellow (as opposed to pure
 * white, which a naive application of the relationship would get).
 *
 * <p>
 * <strong>Note:</strong> It <strong>is</strong> possible to create cyclic references between RelativeColor instances
 * (for example, a RelativeColor that has its own key as one of the keys it should fetch). Don't do that.
 *
 * @author Tim Boudreau
 */
public class RelativeColor implements UIDefaults.LazyValue {
     ...
     * @param target A Color or UIManager key for a color that is what the target color should be if the actual color is
     * equal to the base color
     * @param actual Either a Color object or a UIManager String key resolvable to a color, which represents the actual
     * color, which may or may not match the target color
     * @param mustContrast Either a Color object or a UIManager String key resolvable to a color which must contrast
     * sufficiently with the derived color that text will be readable. This parameter may be null; the others may not.
     */
     ...
     * @param target A Color that is what the target color should be if the actual color is equal to the base color
     * @param actual Either a Color object or a UIManager String key resolvable to a color, which represents the actual
     * color, which may or may not match the target color
     * @param mustContrast Either a Color object or a UIManager String key resolvable to a color which must contrast
     * sufficiently with the derived color that text will be readable
     */
    ...
    /**
     * Convenience getter, as this class is reasonably useful for creating derived colors without putting them into
     * UIDefaults
     */
    ...
    /**
     * Ensures that the key and color are not null, and returns true if the color needs to be loaded.
     */
