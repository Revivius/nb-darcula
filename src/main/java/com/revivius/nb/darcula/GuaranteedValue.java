 ...
 * private. ------------------------------------------------------------
 *
 * A simple mechanism for guaranteeing the presence of a value in UIDefaults. Some look and feels (GTK/Synth) don't
 * necessarily provide these colors, resulting in null pointer exceptions. Since we want to make it easy to write
 * maintainable components, it is preferable to centralize guaranteeing the value here, rather than have the codebase
 * littered with null tests and fallbacks for null colors.
 *
 * It will either take on the existing value if present, or used the passed value if not present. If passed an array of
 * UIManager keys, it will try them in order, and use the first value that returns non-null from
...
public class GuaranteedValue implements UIDefaults.LazyValue {
    ...
    /**
     * Convenience getter of the value as a color - returns null if this instance was used for some other type
     */
